...
<div class="heading" data-sticky="top">
	<div class="row l5 m4 s3 fluid">
		...
		<div class="span l1 m1 s1" style="text-align: right">
			<a class="button" style="font-size:.6em;" href="<?= url('app', 'keygen', $app->_id) ?>">Generate new key</a>
		</div>
	</div>
</div>
...
<?php $keys = db()->table('client\key')->get('client', $app)->group()->where('expires', '>', time())->where('expires', null)->endGroup()->fetchAll(); ?>
<?php foreach ($keys as $key): ?>
<div class="row l5 has-dials">
	<div class="span l4">
		<div style="font-size: .75em; color: #000"><?= $key->expires === null? 'active' : date('Y/m/d', $key->expires) ?></div>
		<div style="font-size: .75em; color: #555"><?= __($key->public) ?></div>
	</div>
</div>

	...
	<div class="span1 material">
			<form method="get" class="regular row l3">
				<div class="field span l1" style="display: inline-block; margin: 0;">
					<input type="text" name="q" placeholder="Search..." 
						<?=isset($_GET['q'])?
							'value="'.htmlspecialchars($_GET['q'], ENT_QUOTES|ENT_HTML5|ENT_SUBSTITUTE).'"':
							''/* don't allow a url XSS attack to target mods*/?>
					/>
				</div>
				<div class="span l2">
		...
		<table>
			...
			<tr>
				...
				<td><?=$user->disabled !== null?
						'<b style="color: #490C0E;">Disabled</b>':
						'<i style="color: #999;">Active</i>'?>
				</td>
				<td style="width: 40%;">
					<?php
						$banned = $user?
					...
					if ($banned) {
						...
						<a href="#" 
							onclick="this.nextElementSibling.style.display = 'block'; this.remove(); return false;"
						>[show]</a>
						<p style="display: none;">
						<?php
							echo (new DateTime('now'))->diff(new DateTime('@'.$banned->expires))
								->format('Expires in: %a days, %h hours and %i minutes').'</br>';
